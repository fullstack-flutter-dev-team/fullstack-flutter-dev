...
// 다양한 방법들 중에 한가지(그나마 쉬운 것을 골라서...)이다
...
public class MyUtil
{
    ...
    /**
     * ■ 전체 페이지 수를 구하는 메소드
     * @param numPerPage - 한 페이지에 표시할 데이터(게시물)의 수
     * @param dataCount - 전체 데이터(게시물)의 수
     * @return
     */
    ...
    /**
     * ■ 페이징 처리 기능의 메소드
     * @param currentPage - 현재 표시할 페이지
     * @param totalPage - 전체 페이지 수
     * @param listUrl - 링크를 설정할 url 
     * @return
     */
    public String pageIndexList(int currentPage, int totalPage, String listUrl) {
        ...
        //-- 페이징 처리 시 게시물 리스트 하단의 숫자를 10개씩 보여 주도록 설
        int numPerBlock = 10;
        
        //-- 현재 페이지(를 셋팅하는 변수) 설정
        // (이 페이지를 기준으로 보여주는 숫자가 달라져야하기 때문에...)
        ...
        /*
            - 클라이언트 요청의 형태 → List.jsp → ..................... + pageNum=5
              ...
              → (결과) → 『List.jsp?pageNum=5』 와 같은 형태
              
            - 클라이언트 요청의 형태 → List.jsp?searchKey=title..........+ pageNum=5
              → (가공) → List.jsp?searchKey=title + 『&』 + pageNum=5
              → (결과) → 『List.jsp?searchKey=title&pageNum=5』 와 같은 형태 
        ...
        //    이와 같은 상황일 경우.... 다시 10을 빼서 10으로 만들어주기 위한 구분
        ...
        while ((page <= totalPage) && (page <= (currentPageSetup+numPerBlock)))
        {
            if (page == currentPage)
            {
                // 현재 페이지 구성 - 현재 있는 페이지라 링크 이동(X), 색상으로 표시
        ...
        // ◈ Next 페이지(다음 페이지로) -- 『Prev』페이지 설명 참조
        n = currentPage + numPerBlock;
