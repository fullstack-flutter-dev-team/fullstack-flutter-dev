...
switch (strOp) {
    ...
    case "-": ret = num1-num2; break;
    case "*": ret = num1*num2; break;
    case "/": ret = num1/(double)num2; break;
}
...
『입력하신 45와(과) 32의 연산 결과는 77 입니다.』
 ...
 <div>
 <% if(strOp.equals("/")) { %>
     입력하신 <span><%=num1 %></span>와(과) <span><%=num2 %></span>의 연산 결과는 <span><%=String.format("%.1f", ret) %></span> 입니다.
 <% } else { %>
     입력하신 <span><%=num1 %></span>와(과) <span><%=num2 %></span>의 연산 결과는 <span><%=(int)ret %></span> 입니다.
 <% }%>
 </div>
