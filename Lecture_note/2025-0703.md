...
- ▣ GROUP BY 쿼리에서 ROLLUP 함수
  -  [[Oracle] 오라클 ROLLUP 함수 사용법 (소계, 합계)](https://gent.tistory.com/57)
  -  ROLLUP 함수는 다차원 집계를 생성하여, 그룹별 합계뿐만 아니라 전체 합계까지 한 번에 계산
  -  소계 또는 합계를 함께 조회해서 데이터를 반환할 수 있기때문에 통계 작업 쿼리를 만들때 아주 유용
  -  ROLL_UP(A)  A 그룹핑 -> 합계
  -  ROLL_UP(A, B)  A, B 그룹핑 -> A소계 / 합계
  -  ROLL_UP(A, B, C)  A, B, C 그룹핑 -> (A소계, B소계) / 합계

- ▣ CUBE() 
  -  ROLLUP() 보다 더 자세한 결과를 반환받을 수 있다.  
