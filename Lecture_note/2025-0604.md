...
### ■■■ ⭐ 정렬(Sort) 알고리즘 ⭐ ■■■
- 정렬 처리는 PC 리소스 소모량이 많기 때문에, 성능 개선을 위한 다양한 알고리즘들이 존재한다.
  ```
  ○ 정렬(Sort)
...
- [[알고리즘/JAVA] 정렬 알고리즘 : 선택, 삽입, 버블, 병합, 퀵 정렬](https://sjh9708.tistory.com/209)
  ...
  - 시간 복잡도가 $O(nlog(n))$인 정렬  : 병합 정렬(항상), 퀵 정렬(일반적으로), 힙 정렬

### ■■■ 정렬(Sort) 알고리즘 ■■■
- **선택 정렬(Selection Sort)**
- ⭐⭐ [**Test111.java**](https://github.com/fullstack-flutter-dev-team/fullstack-flutter-dev/blob/main/JavaStudy/Test111.java)
    ```
          {52, 42, 12, 62, 60}
        ---------------------------------------
          52  42  12  62  60
...
### ■■■ 정렬(Sort) 알고리즘 ■■■
- **버블 정렬(거품 정렬, Bubble Sort)** , 교재 p.108
- ⭐⭐ [**Test112.java**](https://github.com/fullstack-flutter-dev-team/fullstack-flutter-dev/blob/main/JavaStudy/Test112.java)
  ```
        {52, 42, 12, 62, 60}
      -----------------------------------
        52  42  12  62  60
...
- ⭐⭐ [**Test113.java**](https://github.com/fullstack-flutter-dev-team/fullstack-flutter-dev/blob/main/JavaStudy/Test113.java)
  ```
      {62, 12, 20, 22, 30}
    -----------------------------
      61  12  20  22  30
      ...
      12  20  22  30  62
              ==  --
      ------------------------------ 2 회전 (웅~)
                                    swap 발생  → false  → 다음 회전 진행 ❌
  ```
### 	■■■ 정렬(Sort) 알고리즘 ■■■
- ⭐⭐ [**Test114.java**](https://github.com/fullstack-flutter-dev-team/fullstack-flutter-dev/blob/main/JavaStudy/Test114.java)
    ```
...
### ■■■ 클래스 고급 ■■■
-  상속(Inheritance) , 교재(p.194)
-  클래스에 protected 멤버가 있으면, 상속을 고려해서 설계한 클래스라 간주.
-  상속을 코드의 재사용/재활용 관점으로만 생각하지말자.
-  상속과 논리적 포함관계
   -  군인 클래스가 총이라는 클래스를 상속하는 것(Has-A) ❌
   -  학생 클래스가 사람 클래스를 상속하는 것(Is-A 관계) ⭕
- ⭐⭐ [**Test115.java**](https://github.com/fullstack-flutter-dev-team/fullstack-flutter-dev/blob/main/JavaStudy/Test115.java)  
  ```
    ○ 상속(Inheritance)이란
