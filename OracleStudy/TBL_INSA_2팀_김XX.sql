...
ORDER BY (EXTRACT(YEAR FROM SYSDATE) - EXTRACT(YEAR FROM IBSADATE)) DESC;
/* 
이름                   입사일       
-------------------- ----------
김인수               1995-02-23
...
SELECT BASICPAY, DENSE_RANK() OVER (ORDER BY BASICPAY) "순위"
FROM TBL_INSA
WHERE (DENSE_RANK() OVER (ORDER BY BASICPAY) ) <= 5;
...
FROM TBL_INSA
WHERE (SELECT BUSEO, MAX(BASICPAY)
FROM TBL_INSA
GROUP BY BUSEO)
