# Collect earliest and latest git commit dates for each file under OracleStudy (recursive)
...
$out = '.git_dates2\OracleStudy_files.txt'
if(Test-Path $out){ Remove-Item $out }
$files = Get-ChildItem -Path .\OracleStudy -Recurse -File -Force -ErrorAction SilentlyContinue
if(-not $files){ Write-Output "No files found in OracleStudy"; exit }
foreach($f in $files){
    $full = $f.FullName
