# Collect earliest and latest git commit dates for each file under Lecture_note (recursive)
...
$out = '.git_dates2\Lecture_note_files.txt'
if(Test-Path $out){ Remove-Item $out }
$files = Get-ChildItem -Path .\Lecture_note -Recurse -File -Force -ErrorAction SilentlyContinue
if(-not $files){ Write-Output "No files found in Lecture_note"; exit }
foreach($f in $files){
    $full = $f.FullName
